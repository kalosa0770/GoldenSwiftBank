...
const LoginForm = ({ onLoginSuccess }) => {
  ...
  useEffect(() => {
    // Clear status message from state after showing it once
  ...
  const handleBack = () => {
    navigate(-1); // Use navigate(-1) for better history control
  }
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ASSUME the backend returns all required user details:
      const { 
        firstName, 
        lastName, 
        email, 
      ...
      // Show success modal and START the countdown
      setShowSuccessModal(true);
      setModalProgress(100);

      // Delayed navigation/state update until the modal animation finishes
      const interval = setInterval(() => {
        setModalProgress(prev => {
          if (prev <= 0) {
            ...
            // ⭐ CRITICAL FIX: Call onLoginSuccess with full data. 
            // Let the parent (App.jsx) handle the state update AND navigation.
            if (onLoginSuccess) {
              onLoginSuccess(firstName, lastName, email, phoneNumber, isAccountVerified, userId);
            }
  ...
  // handleResendOtp remains the same
  ...
  return (
    <div className="relative flex flex-col min-h-screen w-full items-center justify-center font-sans p-4">
      ...
      <div className="relative z-10 flex flex-col w-full items-center justify-center">
        ...
        <button onClick={handleBack} className='py-3 px-4 bg-amber-700 text-white absolute rounded-2xl left-8 top-8 cursor-pointer z-20 flex gap-1 items-center'> 
