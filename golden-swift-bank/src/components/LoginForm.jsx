  ...
  const handleLogin = async (e) => {
    ...
    try {
      // API call to authenticate
        ...
        // Destructure the token, message, AND the new userName field
        const { token, userName, message } = response?.data; 
        
        if (token && userName) {
            localStorage.setItem('authToken', token);
            localStorage.setItem('userName', userName); // <-- NEW: Store the user's name
            
            setStatusMessage({ text: message || 'Login successful!', type: 'success' });
            setError('');
            if (onLoginSuccess) onLoginSuccess();
        } else {
            // Updated error check if token or name is missing
            setError('Login failed. Server response missing token or user name.');
        }
    } catch (err) {
